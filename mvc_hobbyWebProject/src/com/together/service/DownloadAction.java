...
public class DownloadAction extends HttpServlet {
	...
	protected void actionDo(HttpServletRequest request, HttpServletResponse response)
		...
		String fileUploadName = BoardDAO.getInstance().getFileName(mapper, boardID);
...
//		다운로드 할 파일이 저장된 실제 물리적 디렉토리 경로를 다운로드 할 파일 이름과 연결한다.
		...
		String filePath = request.getSession().getServletContext().getRealPath("/upload/" + fileRealName);
...
//		}
		...
		if (request.getHeader("user-agent").indexOf("MSIE") == -1) {
			fileUploadName = new String(fileUploadName.getBytes("UTF-8"), "8859_1");
		} else {
			fileUploadName = new String(fileUploadName.getBytes("EUC-KR"), "8859_1");
		}
		response.setHeader("Content-Disposition", "attachment;filename=\"" + fileUploadName + "\";");
		...
		/*
		 * /다운로드가 완료되면 board테이블의 다운로드 횟수도 증가시킨다. new
		 * BoardDAO().download(boardID,filename);
		 */
	}
